...
RegresionMultiple::~RegresionMultiple()
{
    delete ui;
}

void RegresionMultiple::metodo(){
    ...
    A[0]=X1;A[1]=X2;A[2]=X3; A[3]=X4; A[4]=X5; A[5]=X6;   //We store the value of x in a vector
    B[0]=F1;B[1]=F2;B[2]=F3; B[3]=F4; B[4]=F5; B[5]=F6;   //We store the value of y in a vector
    ...
    mediaY=sumaY/n; //We obtain the median of y
  ...
  }//We calculate St and Sr
  for (i=0;i<6;i++){
      St=St+E[i];
      Sr=Sr+F[i];
  }
    //We verify Sr equals zero since this form r would become infinite

    if(Sr==0){
    temp.append("\ny=").append(temp2.setNum(a0)).append("+(").append(temp3.setNum(a1)).append(")x1+(").append(temp4.setNum(a2)).append(")x2\nr=Infinito");
}
    //We verify that Sr is different from 0 in this way we can express r
...
void RegresionMultiple::borrar(){ //Delete function
    ui->x1->setValue(0.0);
    ui->x2->setValue(0.0);
    ui->x3->setValue(0.0);
    ui->x4->setValue(0.0);
